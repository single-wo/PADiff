...
BondType = Tuple[int, int, int]  
BondLengthData = Tuple[BondType, float]  
BondLengthProfile = Dict[BondType, np.ndarray]  
...
def plot_distance_hist(pair_length_profile, metrics=None, save_path=None):
    ...
    for idx, (k, gt_distribution) in enumerate(eval_bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items()):
        ...
        if idx == 0:
            plt.xlabel(r'Distance of carbon carbon bond ($\AA$)', fontsize=16)
        elif idx == 1:
            plt.xlabel(r'Distance of all atom pairs ($\AA$)', fontsize=16)
